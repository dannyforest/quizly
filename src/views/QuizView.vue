<template>
  <div id="quiz">
    ...
    <div v-if="quizStarted">
      <Timer ref="timerRef" :duration="selectedTimeLimit" @time-up="endQuiz"/> <!-- 300 seconds = 5 minutes -->
      <ProgressBar :currentQuestionIndex="currentQuestionIndex" :totalQuestions="selectedQuestions.length"/>
      <Quiz
      ...
      />
      <Results v-if="showResults" :score="score" :questions="selectedQuestions" :userAnswers="userAnswers"
               @restart="resetQuiz"/>
    </div>
  </div>
</template>
...
const prepareQuiz = () => {
  ...
  const questions = JSON.parse(localStorage.getItem('questions')) || questionsData;
  const filteredQuestions = questions.filter(question => question.category === selectedCategory.value);
...
const updateHighScore = () => {
  ...
  if (!highScores.value[selectedCategory.value]) {
    ...
    const existingEntryIndex = highScores.value[selectedCategory.value].findIndex(entry => entry.username === username.value);
